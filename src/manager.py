...
def start():
    ...
    if args.cmd == 'self':
        if args.ucci:
            import src.worker.play_with_ucci_engine as self_play
        else:
            if mp.get_start_method() == 'spawn':
                import src.worker.self_play_windows as self_play
            else:
                from src.worker import self_play
        return self_play.start(config)
    elif args.cmd == 'opt':
        from src.worker import optimize
        return optimize.start(config)
    elif args.cmd == 'play':
        if args.cli:
            import src.play_games.play_cli as play
        else:
            from src.play_games import play
        ...
        if args.elo == False:
            from src.worker import evaluator
        else:
            if mp.get_start_method() == 'spawn':
                import src.worker.compute_elo_windows as evaluator
            else:
                import src.worker.compute_elo as evaluator
        config.eval.update_play_config(config.play)
        evaluator.start(config)
    elif args.cmd == 'sl':
        if args.onegreen:
            import src.worker.sl_onegreen as sl
            sl.start(config, args.skip)
        else:
            from src.worker import sl
            sl.start(config)
        
    elif args.cmd == 'ob':
        from src.play_games import ob_self_play
        pwhc = PlayWithHumanConfig()
        pwhc.update_play_config(config.play)
